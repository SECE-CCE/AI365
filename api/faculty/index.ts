  ...
  try {
    ...
    if (normType === 'learning_hour') {
      ...
      if (updatedItem) {
        ...
        // Calculate team author division and auto-create approved learning hours for all student co-authors
        if (action === 'Approved') {
          ...
          const totalAuthorsCount = Math.max(1, 1 + coAuthorsList.length);
          const sharePerAuthor = Math.max(1, Math.round(totalPaperHours / totalAuthorsCount));

          // 1. Find matching registered student accounts for co-authors
          const allStudents = await db.getAllUsers({ role: 'student' });
          const matchedCoAuthorStudents: any[] = [];

          for (const name of coAuthorsList) {
            const cleanName = name.toLowerCase();
            const matched = allStudents.find(
              s => s.id !== studentId && (
                s.full_name.toLowerCase().includes(cleanName) ||
                cleanName.includes(s.full_name.toLowerCase()) ||
                (s.email && s.email.toLowerCase().includes(cleanName)) ||
                (s.register_number && cleanName.includes(s.register_number.toLowerCase()))
              )
            );

            if (matched && !matchedCoAuthorStudents.some(m => m.id === matched.id)) {
              matchedCoAuthorStudents.push(matched);
            }
          }

          // Total registered student authors = submitter + matched registered co-authors
          const registeredStudentCount = 1 + matchedCoAuthorStudents.length;

          // If all co-authors are external/unregistered, credit the full paper hours to the submitter so the total paper effort is preserved
          // Otherwise, credit each registered student author their equal sharePerAuthor
          const submitterCreditedHours = matchedCoAuthorStudents.length === 0 ? totalPaperHours : sharePerAuthor;

          // 2. Credit Submitter
          await db.createLearningHour({
            student_id: studentId,
            activity_name: `Research Paper: ${updatedItem.title} (${submitterCreditedHours}h share)`,
            platform: updatedItem.conference_journal || 'IEEE/Conference',
            date: new Date().toISOString().split('T')[0],
            hours: submitterCreditedHours,
            description: `Awarded ${submitterCreditedHours} hrs (${totalPaperHours} total effort ÷ ${totalAuthorsCount} authors) for research publication "${updatedItem.title}"`,
            certificate_url: updatedItem.pdf_url || '',
            status: 'Approved',
            faculty_id: facultyId,
            faculty_remarks: remarks || `Approved author share: ${submitterCreditedHours}h of ${totalPaperHours}h`,
          });

          // 3. Credit each matched registered student co-author
          for (const coAuthorStudent of matchedCoAuthorStudents) {
            await db.createLearningHour({
              student_id: coAuthorStudent.id,
              activity_name: `Research Paper (Co-Author): ${updatedItem.title} (${sharePerAuthor}h share)`,
              platform: updatedItem.conference_journal || 'IEEE/Conference',
              date: new Date().toISOString().split('T')[0],
              hours: sharePerAuthor,
              description: `Awarded ${sharePerAuthor} hrs as co-author (${totalPaperHours} total effort ÷ ${totalAuthorsCount} authors) for research publication "${updatedItem.title}"`,
              certificate_url: updatedItem.pdf_url || '',
              status: 'Approved',
              faculty_id: facultyId,
              faculty_remarks: remarks || `Approved co-author share: ${sharePerAuthor}h of ${totalPaperHours}h`,
            });

            await db.createNotification({
              user_id: coAuthorStudent.id,
              title: 'Co-Author Credit Awarded!',
              message: `You were credited ${sharePerAuthor} learning hours as co-author on "${updatedItem.title}".`,
              type: 'approval',
            ...
            await db.logActivity(
              facultyId,
              `Approved Co-Author Share`,
              `Credited ${sharePerAuthor} hrs to co-author ${coAuthorStudent.full_name} for "${updatedItem.title}"`,
              coAuthorStudent.id
            );
          }
        }
      ...
      if (updatedItem) {
        ...
        // Auto-create approved learning hours for the student when AI Project is approved!
        if (action === 'Approved') {
          const awardedHours = Number(req.body.awarded_hours || req.body.hours || 40);
          await db.createLearningHour({
            student_id: studentId,
            activity_name: `AI Project: ${updatedItem.title}`,
            platform: updatedItem.tech_stack || 'AI Prototype',
            date: new Date().toISOString().split('T')[0],
            hours: awardedHours,
            description: `Awarded ${awardedHours} verified learning hours for AI project "${updatedItem.title}"`,
            certificate_url: updatedItem.github_link || updatedItem.demo_link || '',
            status: 'Approved',
            faculty_id: facultyId,
            faculty_remarks: remarks || 'Approved by Faculty/Admin',
          });
