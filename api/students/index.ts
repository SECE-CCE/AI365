...
function formatDate(val: any): string {
  ...
  return str.includes('T') ? str.split('T')[0] : str.slice(0, 10);
    ...
    const isAutoGeneratedLearningHour = (activityName: string) =>
      /^(Verified Certificate|Research Paper|AI Project):\s/i.test(activityName || '');
    const pointsForLearningHours = (hours: any) => {
      const marks = parseAdminMarks(hours.admin_marks);
      return marks !== null ? marks : Number(hours.hours || 0) * 2;
    };
    const pointsForApprovedWork = (item: any, type: 'certificate' | 'research' | 'project', defaultPoints: number) => {
      const marks = parseAdminMarks(item.admin_marks);
      const prefix = type === 'certificate' ? 'Verified Certificate' : type === 'research' ? 'Research Paper' : 'AI Project';
      const creditedHours = learningHours
        .filter(hour => hour.activity_name === `${prefix}: ${item.title}`)
        .reduce((total, hour) => total + Number(hour.hours || 0), 0);
      const learningHoursPoints = creditedHours * 2;
      return {
        allocated_hours: creditedHours,
        learning_hours_points: learningHoursPoints,
        total_points_awarded: (marks !== null ? marks : defaultPoints) + learningHoursPoints,
      };
    };

    const recentActivities = [
      ...learningHours
        .filter(lh => !isAutoGeneratedLearningHour(lh.activity_name))
        .map(lh => ({
          id: `lh-${lh.id}`,
          type: 'Learning Hours',
          activity: lh.activity_name,
          hours: `${lh.hours} hrs`,
          allocated_hours: Number(lh.hours || 0),
          date: formatDate(lh.date),
          status: lh.status,
          admin_marks: parseAdminMarks(lh.admin_marks),
          learning_hours_points: pointsForLearningHours(lh),
          total_points_awarded: pointsForLearningHours(lh),
      ...
      ...certificates.map(c => ({
        ...
        ...pointsForApprovedWork(c, 'certificate', 50),
      ...
      ...researchPapers.map(p => ({
        ...
        ...pointsForApprovedWork(p, 'research', 150),
      ...
      ...projects.map(proj => ({
        ...
        ...pointsForApprovedWork(proj, 'project', 100),
